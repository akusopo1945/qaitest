...
test("qa dashboard renders the planning gui", async ({ page }) => {
  ...
  await expect(page.getByTestId("qa-demo-provider-badge")).toContainText("OpenAI tuned");
});
...
test("qa demo run stays interactive and always ends in success", async ({ page }) => {
  ...
  const providerBadge = page.getByTestId("qa-demo-provider-badge");
  const modelLabel = page.getByTestId("qa-model-label");
  const modelInput = page.getByTestId("qa-model-input");
  const demoShell = page.getByTestId("qa-demo-run");
  ...
  await expect(providerBadge).toContainText("Azure tuned");
  await expect(demoShell).toHaveAttribute("data-provider-theme", "Microsoft Azure OpenAI API");
